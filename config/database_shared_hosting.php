...
class Database {
    ...
    /**
     * Executa o script de criação do banco (versão para hospedagem compartilhada)
     */
    public function createDatabase() {
        try {
            // Usar o arquivo específico para hospedagem compartilhada
            $sqlFile = __DIR__ . '/../database/schema_no_create_db.sql';
            if (!file_exists($sqlFile)) {
                throw new Exception("Arquivo schema_no_create_db.sql não encontrado");
            }
            
            $sql = file_get_contents($sqlFile);
            
            // Separar comandos SQL por ';' e executar um por vez
            $commands = $this->parseSQLCommands($sql);
            ...
            foreach ($commands as $command) {
                if (!empty(trim($command))) {
                    try {
                        ...
                        // Log do erro mas continua (algumas constraints podem falhar se já existirem)
                        error_log("Aviso SQL: " . $e->getMessage() . " - Comando: " . substr($command, 0, 100));
                    }
                }
            }
            ...
            throw new Exception("Erro ao criar estrutura do banco: " . $e->getMessage());
        }
    }
    
    /**
     * Faz parse dos comandos SQL considerando delimitadores
     */
    private function parseSQLCommands($sql) {
        // Remover comentários
        $sql = preg_replace('/--.*$/m', '', $sql);
        $sql = preg_replace('/\/\*.*?\*\//s', '', $sql);
        
        // Separar por ; mas considerar que pode haver ; dentro de strings
        $commands = [];
        $current = '';
        $inString = false;
        $stringChar = null;
        ...
        return array_filter($commands, function($cmd) {
            return !empty(trim($cmd)) && !preg_match('/^\s*(USE|CREATE\s+DATABASE)/i', $cmd);
        });
    }
